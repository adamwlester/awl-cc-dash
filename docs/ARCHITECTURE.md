...
Built (§11 #50, 2026-07-16): the close semantics (`POST /projects/close` with `stop_agents`), the styled two-option confirm in Settings → Projects, and the app-quit path — Electron main asks the renderer to show the same styled dialog over IPC and falls back to the native dialog when the renderer can't answer (no ack, reload, or crash mid-dialog). Proven end-to-end in the real Electron window (e2e, 2026-07-16, Playwright-Electron with real clicks): the styled dialog appears on window-close over IPC, "Close" quits and terminates an **owned** sidecar, and an **adopted** sidecar survives the same flow untouched.
...
where projects live. Built (§11 #26, 2026-07-10): `sidecar/runtime/projects.json`, maintained by the register/open/close endpoints and feeding the picker. Index rows carry a **`registered`** flag (2026-07-16 gap batch): only the operator's explicit acts (`POST /projects/register` / `POST /projects/open`) register a row, and only registered rows feed the picker; an implicit touch by record routing (e.g. a fork's git-worktree cwd) creates a hidden-but-enumerable `registered: false` row — the fork's records stay cold-restorable, retireable, and archivable without the worktree ever surfacing as a known project. Pre-flag rows are grandfathered visible, and an implicit touch never downgrades registration.
...
The renderer is the #37 rebuild (§4.4) — built fresh from the `design/` system, its DOM and CSS mirroring `mockup.html`. [`main.tsx`](../frontend/src/renderer/main.tsx) mounts [`App.tsx`](../frontend/src/renderer/App.tsx), a thin shell reproducing the design frame — title bar · three resizable columns (Agent | Team / Library | Feed / Prompt — the **Team** panel (formerly Team Graph) and the **Feed** panel (formerly Team Feed), renamed 2026-07-18) · status footer — plus the Settings and Console step-into views. The pieces, all under `frontend/src/renderer/`:

| Component | File | Role |
|-----------|------|------|
| `AgentPanel` | `components/AgentPanel.tsx` | Left pane — Details · Create · Console tabs for the focused agent (view-only identity per the 2026-07-17 Create-lock §7.5; the live mode/model/effort/fast/thinking levers, Retire/Delete, the #13 arm-gated mode ring; the former Past tab moved to the Team panel's Past drawer, 2026-07-18) |
| `TeamGraph` | `components/TeamGraph.tsx` | The Team panel — agent cards: identity row, status/model badges, Ctx/Turns bars, run-strip, marquee, subagent strip |
| `TeamFeed` | `components/TeamFeed.tsx` | The Feed panel (right-top) — Transcript · Scratch · Log · Inbox over the shared From/To filter accordion |
| `PromptPanel` | `components/PromptPanel.tsx` | Right-bottom — Compose (editor, templates, inserted blocks, Attach, send-timing, Revise) + History |
| `Library` | `components/Library.tsx` | Middle-bottom — Plans · Documents · Assets as reviewable-doc cards (sidecar comments, raw-markdown edit), plus the header Import control (§11 #28) |
| `LinkDrawer` / `PastDrawer` | `components/LinkDrawer.tsx` · `components/PastDrawer.tsx` | The Team panel's two mutually-exclusive sheet drawers, opening against the column's right edge — Link Config, and the Past drawer (the former Agent-pane Past tab, moved 2026-07-18: the §11 #17 resume picker + the §7.12 archive roster, same content against the same endpoints — `GET /sessions/past` · `POST /sessions/resume` · `GET /archive` · `DELETE /archive/{id}` — at the tokenized wider 416px) |
| `SettingsView` / `ConsoleView` | `components/SettingsView.tsx` / `Console.tsx` | The step-into views — Settings (Projects · Setups · Usage · MCP · Plugins · Config, §3.2) and the xterm live Console (§7.13; the in-column Console tab shares its internals) |
| Frame + shared bits | `components/frame.tsx` · `selectors.tsx` · `ExportControl.tsx` | Title bar / footer / resize splitters · the From/To roster drawers · the merged Export control |
| `api.ts` | `api.ts` | The HTTP/SSE client + the whole endpoint catalog — the contract preserved through the rebuild (§4.4) |
| `design/` | `design/tokens.css` · `styles.css` · `app.css` · `sprite.ts` | Verbatim copies of the design system's tokens + component CSS, the renderer-only shims (`app.css` — filling the window is the one sanctioned divergence from the mockup's fixed dev frame), and the agent-icon sprite extracted from `mockup.html` |
| `lib/` | `lib/identity.tsx` · `transcript.ts` · `icons.tsx` · `toast.ts` | Shared vocabulary — roster identity rendering, merged-bus events → transcript/log card derivation, Lucide + sprite icons, toasts |

**State** lives in [`store.tsx`](../frontend/src/renderer/store.tsx) — one React context (`DashProvider`/`useDash`; no Redux/Zustand). The store owns the merged `/events` SSE subscription (backfill + dedup by `id`, ordered by `seq`, ~4000-event client cap), every §4.3 polling cadence, the #38 degraded mode (freeze-on-stale, backoff, the title-bar connector-health badge), and the client-side subagents-blend backstop (`normalizeSubs`, §7.17); components read live data through it and hold only local UI state.
...
- **Targeted polling (pull)** for readouts that aren't event-shaped, each on its own cadence:
  ...
  **scratchpad** is deliberately **not** on a poll — it is read on demand, and its deltas are pushed to
  ...
  agent's terminal is a **live streaming attach** (`ttyd`/WebSocket), decided 2026-07-05 and built (§7.13, §11 #29 — backend attach 2026-07-14, the xterm renderer 2026-07-15, e2e-proven).

Failure handling is **decided (2026-07-05) and built (§11 #38, 2026-07-15, e2e-verified)**: on `/health` failure the poll-driven panels **freeze on last-known values, visibly marked stale**, and polling **backs off** to a gentle retry until the sidecar returns; the SSE reconnect and the connector-health badge live in the store transport ([`store.tsx`](../frontend/src/renderer/store.tsx)). The *consolidated* always-visible system-health indicator is a **design-lane** item: today the health signals are scattered across three surfaces — the Settings connector badges, the System Error/Warning inbox cards (§7.8), and the "Sidecar offline" chip — and the operator wants them consolidated into one always-visible indicator in the app chrome (leading candidate surface: the **footer**), with a broadened state vocabulary (adding *down* + *stale/degraded*), a popover drill-in (a sidecar-log tail, §11 #43, is a candidate), reconciled with the separately-captured title-bar Connected-chip upgrade note.

### 4.4 Frontend build strategy — the renderer was rebuilt fresh

**The prototype renderer was throwaway — by strategy, not accident.** The first React renderer was an early prototype, never the shippable client. Rather than finishing or porting it in place, the recorded strategy was to **freeze** it and **rebuild the visible renderer fresh from the design system** (`design/` — authority `mockup.html`, values in `tokens.css`) once, from scratch — so the design lane could keep evolving the mockup as the single source of truth without a live port dragging behind it. The one artifact carried through the rebuild was [`api.ts`](../frontend/src/renderer/api.ts) — the frontend↔sidecar contract (§4.3, §5.2) — and the standalone Playwright **`tests/ui/`** slice proved "a client can drive the live loop" against that contract while the prototype sat parked.

**Scope of that freeze — renderer only, never the Electron shell.** What was frozen was the **visible UI**: layout, styling, components, and their interaction behaviour — the surface the design system owns. The Electron **main-process shell** was never frozen and is not design-owned: sidecar spawn/supervise/shutdown (proven in the Python model, §2, then ported into Electron main — §11 #20, built 2026-07-16), window + app lifecycle, detach-on-close, and packaging carried their own feasibility track throughout. "Don't build the frontend yet" always meant the renderer UI — never the shell plumbing.

The renderer **rebuild is done** (§11 #37, built 2026-07-15): the visible renderer was rebuilt fresh from `design/`, wired to the built backends, and e2e-proven (25 colours, marquee, the live xterm Console). The `tests/ui/` slice exists (built in the 2026-07-02 spike batch). The **#50 wiring half is also done** (2026-07-16, headed-verified against a live sidecar with real agents): Projects (chip / picker / close-confirm / picker-first startup), Bypass/Auto launch-arm gating, the Past tab (resume + archive), the Review card, Import, the subagent activity dot, the Timeline with Rewind/Handoff, and Attach/Assets. Both mechanism drives are **e2e-proven in the real Electron window** (2026-07-16, Playwright-Electron: the interactive close dialog in both owned- and adopted-sidecar modes; a real PNG thumbnail via the `nativeImage` IPC), and the backend gaps the wiring surfaced were fixed the same day (the phase-3 gap batch). The #46 rewind anchor landed 2026-07-16/17 (§7.19): the Timeline's rolled marking is server truth and survives reload — headed-proven.
...
| Concern | Endpoints |
|---------|-----------|
| **Health / sessions** | `GET /health` · `GET /system-check` (one honest JSON over the existing probes, §11 #49) · `POST /sessions` · `GET /sessions[/{id}]` · `DELETE /sessions/{id}` (`?hard=true` → permanent wipe + tombstone, §7.12) · `GET /sessions/past` + `POST /sessions/resume` (on-demand resume of dead-roster and archived agents, §11 #17) |
| **Archive** | `GET /archive` · `GET /archive/{id}` · `DELETE /archive/{id}` — the retire deep-freeze table (§7.12, §11 #18); resume-from-archive un-retires via `/sessions/resume` |
| **Messaging** | `POST /sessions/{id}/send` (disposition: now/next/queue/hold/inject) · `GET /sessions/{id}/history` |
| **Merged feed** | `GET /events` (SSE) · `GET /events/history?since=<seq>` — both with server-side From/To filtering · `GET /sessions/{id}/events` (per-session SSE — exists, but the client subscribes to the merged bus, §4.3) |
| **Hook channel** | `POST /internal/hooks/{post-tool-use,stop,plan,decision,workflow,run-state,subagent}/{agent}` (§7.4) · `GET /internal/debug/run-state/{agent}` (arbiter readout, dev/debug) |
...
| **Library** | `GET /library/documents` · `GET /library/document` · `POST /library/document` (create) · `PUT /library/document` (edit-in-place, §7.16) · `DELETE /library/document` (delete the `.md` + its paired `.meta.json`) · rename + comment add/resolve (§8.5 sidecars) · `GET/POST /library/reviews` · `POST /library/assets` (attachment/citation ingest, §7.14) · `GET /library/assets` (asset listing + per-receiver renderings, §7.16) · `DELETE /library/assets/{asset_id}` (remove one ingested asset — traversal-safe, both write legs, §7.16) |
| **Console** | `GET /console/catalog` · `POST /sessions/{id}/console/run` · `POST /sessions/{id}/console/{attach,detach}` (the live streaming terminal — geometry-pinned `ttyd`/WebSocket attach, §7.13; consumed by the rebuilt renderer's xterm Console, e2e-proven 2026-07-15) · `POST /sessions/{id}/console/resize` (the ONE sanctioned, clamped geometry write on the pinned pane, §7.13; built 2026-07-17) |
| **Readouts** | `GET /sessions/{id}/{context,context/breakdown,cost,subagents,checklist,marquee}` · `GET /usage` |
| **Timeline / rewind** | `GET /sessions/{id}/timeline` (per-turn records + server-replayed rolled state, §7.19/§11 #46) · `POST /sessions/{id}/rewind` · `POST /sessions/{id}/fork` (branch-from-N, §11 #15) · `POST /sessions/{id}/handoff-report` (the utility-LLM handoff doc, §11 #16) |
| **Session control** | `POST /sessions/{id}/{interrupt,model,mode,permission,effort,fast,thinking}` — mode/fast/thinking drive the wired `keys()` levers and return the read-back state (409 `busy` / 400 `unreachable`·`credit_gated`, §6.2) · `POST /sessions/{id}/identity` (the §7.5 identity-edit machinery + `/rename` sync — its UI surface was retired by the 2026-07-17 Create-lock; the endpoint stays a sidecar capability) · `GET /identity/random-name?exclude=` (the §7.5 randomize draw, §11 #40) · `POST /sessions/{id}/plan/verdict` (§7.16) · `POST /sessions/{id}/git` (operator-triggered status \| diff \| commit, §11 #47) |
...
| **Roles** | `GET /roles?cwd=` — the two-scope `agent.md` role-preset catalog behind the Create Role combobox (System `~/.claude/agents` · Project `<project>/.awl-cc-dash/agents/`, §7.5/§11 #53) |
| **Prompt library / presets** | `GET /prompt-library?scope=resolved\|defaults\|project` · `POST /prompt-library` (project writes only, §11 #45) · `GET /presets/response` · `GET/POST /sessions/{id}/response-preset` (the per-agent Response preset, §7.14/§11 #39) |
| **Import** | `GET/POST /import/external` — external Claude session import via the context-extractor exporters, to an agent's queue / operator read / a Library doc (§11 #28) |
| **Projects** | `GET /projects` (picker feed + open flag) · `POST /projects/register` · `POST /projects/open` (409 when another is open) · `POST /projects/close` (`stop_agents` = the §3.4 second option) · `POST /projects/changelog/refresh` (re-render the #19-author change-log doc, §11 #48) |
...
| **Assets** | `GET /assets/agent-icons` — the full shipped icon-stem listing feeding the manual picker (§7.5, §11 #56) · `GET /assets/agent-icons/{name}?color=&layer=` — recolorable agent SVGs (§7.5; default recolors the background tile, `layer=glyph` knocks the tile out and tints the glyph — the graph-watermark polarity) · `GET /assets/{asset_id}/{filename}?cwd=` — project-asset byte streaming with the stored MIME (the renderer's render path, §7.14; registered after the icon routes so icons keep precedence) |
...
- **Plan / Decision** PreToolUse hooks surface the agent's `ExitPlanMode` / `AskUserQuestion` tool calls to
  ...
  approve→resume wiring is built (§11 #22, 2026-07-10), with both verdict legs live-proven end to end 2026-07-17 (§7.16).
...
Agent identity is **role + number + name + colour + icon** — set at Create, persisted with the roster (§8.2), shown everywhere, and **locked once the session exists** (operator decision, 2026-07-17, superseding the original all-five-fields-editable model): identity *and* launch config are Create-time facts, and the Agent→Details tab is fully **view-only** — the mockup and the rebuilt renderer both enforce it (every edit pencil is gone; Model, Mode/Effort, and the lifecycle caps stay §7.15's deliberate live-edit exceptions). Identity is dashboard-owned **display metadata**: routing, links, hooks, and the inbox all key on a stable internal session id, never on the name or number. The **name** is additionally registered as the Claude Code session's own display name — set at launch via the `claude --name` flag (the flag exists on CC 2.1.206), live-proven with a read-back from `~/.claude/sessions/<pid>.json` (`test_identity_rename_live`) — so it surfaces in the VS Code extension's session list and the `--resume` picker, not only inside the dashboard. The identity-edit machinery built as §11 #14 (2026-07-10) — `POST /sessions/{id}/identity` merging any field subset with retired numbers refused, plus the `/rename` live-edit sync — remains built and proven at the sidecar, but the 2026-07-17 lock retired its product surface: nothing in the UI calls it anymore.

Every field a Create omits gets a sane default. Auto-assign pools are **25 colours and 50 curated icons**, round-robin (`colour = n mod 25`, `icon = n mod 50`); past 16 agents the icon becomes the primary disambiguator. Icons are recolorable SVGs served from `assets/icons/agents/` via `GET /assets/agent-icons/{name}?color=` — by default the colour fills the background tile under the white glyph; `layer=glyph` flips the polarity (tile knocked out, glyph tinted) for the graph cards' behind-content watermark, design-correct for all 167 shipped icons. The **manual icon picker offers the full shipped set, not just the curated 50** (§11 #56, built 2026-07-18): `GET /assets/agent-icons` lists every stem on disk (167 today — `AG_ICONS` in [`sidecar/identity.py`](../sidecar/identity.py), discovered at sidecar start), and the picker renders sprite-embedded names via the fast in-page sprite and the rest as recolor-endpoint tiles, degrading to the sprite-embedded 50 if the listing fetch fails — never an empty picker. The renderer also reconciles stem spellings across that seam: the curated pool's raw `__author`-suffixed file names alias onto their plain sprite keys, and the missing-sprite fallback paints with the **agent's own colour** rather than hardcoded white (the invisible white-on-white tile that hid 11 curated stems — fixed 2026-07-18). A blank or absent **name** at create **auto-draws from the curated name pool** (2026-07-17 — `assign_identity` in [`sidecar/identity.py`](../sidecar/identity.py), with live agents' names excluded from the draw; an explicit name always passes through untouched): a nameless agent previously persisted `""` and rendered as a bare session id. The pool is the shipped [`assets/names/agent-names.json`](../assets/names/agent-names.json) — 179 one-word, 3–5-letter, lowercase names, validated to double safely as git commit-author names (§11 #19) — backing both the auto-draw and the Create panel's randomize affordance (`GET /identity/random-name?exclude=`, §11 #40, built 2026-07-15), with user-typed names always available. The Create panel's **No.** field defaults blank with an `auto` placeholder — the create payload carries a number only when the user actually typed one, so the sidecar's assignment (which skips retired numbers, §7.12) is the default path (trued 2026-07-17, superseding the old next-in-role prefill, which let a UI create claim a retired low number). The rebuilt renderer ships the full 25-colour pool (§11 #37, built 2026-07-15).

**The model default is the product's own — Opus (operator decision, 2026-07-18; a decision change, additive to the doctrine above).** The product no longer launches agents on whatever Claude Code's own config would default to: the old "Inherit" option — no `--model` flag, engine's choice — is retired from the UI entirely, and both model bands (Agent→Details and Create) offer exactly **Opus · Sonnet · Haiku · Fable**. The Create panel opens Opus-active, and the sidecar enforces the default at the API: `create_session` coerces a missing, empty, or any-case `inherit` model to the module constant `DEFAULT_MODEL = "opus"` ([`sidecar/main.py`](../sidecar/main.py)), so every **new** session persists a concrete model and the bridge always passes `--model`. Handoff/fork parent-inheritance is a different concept — the parent's *concrete* model — and stays server-side, deliberately un-coerced (`req.model or source.model` on the fork path), with the Create-tab handoff prefill selecting the source agent's concrete model rather than an abstract "inherit". Legacy null-model records are **not** rewritten — read-backs stay honest and the UI shows "—", never a fake-Opus highlight. (This repo's five seeded role presets, [`.awl-cc-dash/agents/`](../.awl-cc-dash/agents/), hard-pin `model: fable` — a preset's or user's explicit model always passes through untouched.)

**Role presets — the `agent.md` loader (built 2026-07-17, §11 #53, live-proven).** The Create panel's Role combobox is the **agent.md preset loader** (DESIGN.md's ND 12): `GET /roles` serves a **two-scope catalog** — **System** = `~/.claude/agents`, the persistent cross-project store, surfaced per the 🔌 doctrine (read in place, never owned or copied; `AWL_SYSTEM_AGENTS_DIR` overrides the location for tests) — and **Project** = `<project>/.awl-cc-dash/agents/` (`storage.agents_dir`, the new §8.2 subdir: project role presets committed with the repo). Each entry is the file's front matter, parsed by [`sidecar/roles.py`](../sidecar/roles.py)'s deliberately small hand parser (no PyYAML — the observed corpus is a shallow subset: `---` fences, scalars, inline comma lists, block lists at any indent, quoted scalars incl. quoted hex colors, `#`-commented keys skipped), served snake_case like the rest of the API (`permission_mode`/`max_turns`, read from the files' camelCase `permissionMode`/`maxTurns` spellings). Picking a preset **prefills the Create fields — including colour**: a front-matter `color:` maps Claude Code's named-colour convention onto the identity palette (`NAMED_COLOR_TO_HEX` in `roles.py` — mirrored name-for-name by `design/behavior.js`'s `AG_FROM_NAMED`; the sidecar is the mapping authority) and a raw `#rrggbb` passes through. The **icon is never prefilled** — deliberately: colour groups a role's agents into a family, and the icon is the per-agent differentiator among same-role agents, always hand-chosen at Create. **Prefill-only in v1:** the preset shapes the Create form, not the launched agent — the bridge driver ignores `agent_type` (only the limited-use `sdk` driver maps it to `--agent`), so loading a persona into the running agent itself is a later step, not a current claim.
...
Permission **mode** changes live, mid-run: the bridge drives **Shift+Tab at a known-idle screen** and reads the resulting mode back from the status line (`test_permission_mode_cycle_live`, live, 2026-07-02; wired end to end §11 #12, built 2026-07-10 — one of §6.2's three live controls). **Bypass/Auto are launch-gated:** a Bypass segment that was not pre-armed at launch is **silently absent** from the mode ring — not a visible no-op; it simply isn't reachable (`test_bypass_auto_preconditions_live`, live, 5-case launch matrix) — so the Create panel must set the launch flags and the mode control must disable/hide un-armed segments (§11 #13), never presenting a control that silently does nothing. The UI half is wired (2026-07-16, #50): the Create arm-box expresses arming through the launch mode itself or the arm-only flag below (with an honest-limit note for the one genuinely unreachable case, Auto), and un-armed segments are absent from the Details ring — seeded from the session's `armed_modes` and corrected by the endpoint's honest 400 "unreachable". The backend half landed with the 2026-07-16 gap batch: every session dict exposes a derived **`armed_modes`** set (the base Shift+Tab ring plus Bypass when pre-armed; the `arm_bypass` launch fact persists through the roster and archive records, so reconnect/resume/un-retire re-derive the same ring), **`CreateSessionRequest.arm_bypass`** carries arm-without-activate (`--allow-dangerously-skip-permissions`, live-proven, reapplied on cold restore), and the live set-mode accepts the **`auto`** spelling (`dontAsk` folds to the same segment). Auto's account-eligibility dependence keeps the live 400 as the honest corrector. **Bypass is the default launch mode** (operator decision, 2026-07-17 — superseding the earlier Create-always-starts-un-armed default): `DriverConfig` and `CreateSessionRequest` both default `permission_mode` to `bypassPermissions` (launching in it arms the Bypass segment implicitly, so a default create needs no `arm_bypass`), and the Create panel opens with the Arm Bypass switch on and the Bypass segment selected. The historical read-back fallbacks (`rec.get(..., "acceptEdits")` on roster/archive reads) are deliberately unchanged — they describe what old records were launched with, not the default. The launch-gating truth above is untouched: an un-armed segment is still absent from the ring, and arming remains a launch-time act. Per-agent tool scoping is **deny-based**, because `--allowedTools` is ignored under bypass mode — a known Claude bug.
...
- Wiped: the runtime record, the tmux session, the on-disk transcripts including subagents, the per-agent launch-config dir (the `turns.jsonl` Timeline + `statusline.jsonl` stores — Retire and stop deliberately KEEP it so a resume re-attaches the Timeline, §7.19; the archive row's true-delete purges it too, since nothing can resume the row afterwards), and the
...
**Archive-on-retire is gated, not best-effort (2026-07-17 — the retire data-loss fix, live-proven):** `save_archive_record` ([`sidecar/state_store.py`](../sidecar/state_store.py)) **raises on a no-op** (no `archive_id`, or no on-disk project home) instead of returning silently, and the retire **teardown is gated on the archive actually landing**. Success takes `driver.close()` as before — safe, because the roster row it removes is covered by the standing archive row. A failed write takes the **overridden `stop()` fallback** instead: end the process, KEEP the persisted roster row, so the agent survives as a resumable Past entry (a driver with no stop/close split — the sdk driver — closes record-safe, since its `close()` never touches persisted records; `AgentDriver.stop()` is a deliberate base no-op precisely so only an override counts as the record-keeping path). The response says which path ran: `archived` (the archive id, or null), `archive_error` (the freeze did not land — the roster row was kept), `archive_skipped: "no-project"` (a cwd-less agent is by-design unarchivable, §8.2), and `record_kept` on the non-archived paths. Archive rows are **idempotent per session** — one session, at most one row: a save replaces any standing row carrying the same `session_id` (a retire→failed-resume→retire cycle used to mint a sibling row, live-observed 2026-07-17), and `GET /sessions/past` dedupes per session id as the read-side guard.
...
- **Attach-on-open, detach-on-close:** the live client attaches only while the Console tab is open on that agent (never a live terminal per agent across the fleet); one bounded scrollback catch-up on open. The renderer's two console views — the in-column tab and the expanded step-into — share **one session-keyed attach** (built 2026-07-18): the attach result is cached per session (the in-flight promise included, so a mount+expand race shares one POST), and expanding an already-attached console reuses the live socket with **zero re-attach POST** (~133 ms to the expanded terminal vs ~2 s for a cold attach, live-measured 2026-07-20); the cached attach is invalidated on socket death, retire, hard delete, and resume — a resumed agent always re-attaches fresh, since the recycled port could even be another agent's terminal. The single-writer resize rule below is unchanged by the reuse.
- **Geometry pinning (required):** the agent's tmux pane is pinned via `window-size manual` so an attached viewer cannot resize it and perturb the sidecar's capture-pane coordination reads — the one coexistence hazard, and its fix (naive `window-size latest` lets a viewer resize the pane, and the resize **persists** after it detaches). The pin regime **stands** — and honoring a viewer's geometry is now a **deliberate, sidecar-mediated act**, never a viewer side effect (built 2026-07-17): `TmuxBridge.console_resize` issues `tmux resize-window -x -y` — which preserves `window-size manual` by tmux semantics — clamped to **60–500 cols × 15–200 rows** (the floor is load-bearing: below ~60 columns the TUI wraps the fixed phrases the screen-scraper matches on), behind `POST /sessions/{id}/console/resize` (clamped again endpoint-side, applied values echoed, honest 409 `busy`/400 mapping). The renderer's consoles drive it debounced (~350 ms) under a **single-writer rule** — the expanded step-into view wins while open, and collapsing re-asserts the in-column geometry — so the expanded Console genuinely fills its panel (live-proven 211×58 at a 2560px window).
...
marked honestly in the UI: mid-run permission-mode cycling is proven and wired (§7.11, §11 #12, built 2026-07-10), with un-armed Bypass/Auto segments absent from the mode ring (§7.11, §11 #13); per-agent MCP/model/plugins
...
the head of the prompt queue. **Both legs are live-proven end to end** (2026-07-17, `test_plan_verdict_revise_then_approve_live` on a sidecar-owned real plan-mode agent, CC 2.1.212): revise's Escape dismissed the parked review box AND kept the agent in plan mode, the queued feedback flushed and the agent produced a revised plan containing the instructed change, and approve then executed the plan for real. Two CC 2.1.212 facts from that proof: the plan-review menu changed ("No, keep planning" is gone — option 4 is now "Tell Claude what to change", and Escape still maps to keep-planning), and **the approve default (Enter) is now "Yes, and use auto mode"** — an endpoint-approved agent lands in `auto` permission mode, not the mode it planned in. Edit-in-place ships as `PUT /library/document` (store-scoped). ⚠ **Today:** on CC 2.1.212 the parked plan-review box no longer classifies as `permission_prompt` — the bridge status reads `idle` while the box is parked (likely the box's trailing hint lines outrun `parse_permission_prompt`'s bottom-anchor slack) — benign for the verdict endpoint (it Escapes before any flush), but a user-queued prompt would flush into a parked plan box; the status parser needs a plan-box case.

Listing is **recursive** for the store's `plans/`/`docs/` collections (built 2026-07-16; [`sidecar/library.py`](../sidecar/library.py)): nested trees like `plans/phase-1/plan.md` list with a base-relative `rel_path` per entry (the store dirs list first, with the legacy `<root>/<subdir>` fallback walking the same way; the no-subdir project-root browse deliberately stays top-level — walking a whole repo would be pathological). The recursive scope is one decision applied uniformly — the reviews aggregate and bare-filename review/comment resolution reach the same nested docs, so a doc the listing offers accepts and surfaces comments (never list-then-404 or save-then-vanish) — the walk is **cycle-safe** (directory junctions/symlinks are never traversed; `rglob` would loop forever on a junction cycle and hang the sidecar — regression-proven in `test_library_unit`), and the docs collection **excludes its `docs/prompts/` subtree** (the §11 #45 prompt-library project copy is `/prompt-library`'s data, not documents — listing it would offer prompt overrides as ordinary deletable docs). The **Assets surface's data is served**: `GET /library/assets` lists every project asset with metadata + both per-receiver renderings (loose hand-dropped `assets/` files list honestly with `id: null` — visible media, not byte-endpoint-addressable), and `GET /assets/{asset_id}/{filename}` streams the bytes (§7.14). The Assets **tab UI is wired** (2026-07-16, #50): the rail lists real records (mime/size/provenance, loose-file honesty), previews render the real bytes over the localhost URL, Export is Attach-only per design, and the thumbnail slot rides the decided `nativeImage`/`getFileIcon` IPC with the byte-endpoint image / file-type icon as the renderer fallback. **Asset delete ships** (2026-07-16 gap batch): `DELETE /library/assets/{asset_id}` removes one ingested asset's id-keyed dir + `.meta.json` (traversal-safe, Windows + in-WSL removal legs matching the write legs, honest 400/404/500), and the preview's Remove is enabled behind an inline danger confirm (loose files stay disabled with an honest title). The thumbnail IPC is e2e-proven in the real Electron renderer (2026-07-16 — the Assets tab rendered a real PNG via `nativeImage.createThumbnailFromPath`).
...
(`test_subagents_naming_unit`). The roster surfaces are wired (2026-07-16, #50): the collapsed-parent activity dot, the nested filter tree, the graph-card badge strip, and the Details accordion all read the blend. The blend itself is now **sidecar-side** (2026-07-16 gap batch, `subagents_naming.blend_live`): exact/prefix engine-id merge; a RUNNING hook record pairs in order with the id-less running spawn its result hasn't named yet (no `{id: null}` double-count — the live-caught duplication); still-running hook-only records keep an honest minted id; stopped hook-only leftovers (the engine's internal helper agents, verified live 2026-07-16: 4 such records beside 2 real spawns) are dropped. The rules mirror the live-verified renderer repair, which stays in place as a no-op backstop (`normalizeSubs` in [`store.tsx`](../frontend/src/renderer/store.tsx)).
...
From the Agent→Details **Timeline**, **Rewind** rolls an agent back to a chosen prior message and resumes from there; **Handoff** branches from a chosen point into a *new* agent carrying that conversation prefix. Both are **proven end-to-end** (`test_rewind_handoff_live`, live, 2026-07-02; research: [`s10-research-15-rewind-handoff.md`](../dev/notes/research/s10-research-15-rewind-handoff.md)) on the TUI-native path: **`/rewind`** restores conversation state (not just files) to any prior prompt checkpoint, and **`--fork-session` + `/rewind` inside the fork** is the branch-from-N mechanism. Handoff creates the new agent through the standard Create flow with a **prepopulated Create tab** (§9.2, §7.5). Transcript surgery is ruled out (fragile, unsupported); the Python SDK lacks `resume_session_at` parity, so the TUI-native path is the build path. Two caveats carry into the build (§11 #15): a conversation fork does **not** isolate filesystem state — the build needs an explicit per-fork file-state policy (git worktree / code-checkpoint) — and a **≥ v2.1.191** version gate is required to rewind past a `/clear`. The per-turn settings + summary capture the Timeline rows render is **built** (§11 #46, 2026-07-16 — settings-at-turn + one-line summary per dashboard turn, persisted thin per agent, served by `GET /sessions/{id}/timeline`); the Handoff summary-artifact half is another (§11 #16). Timeline visuals are DESIGN.md's. The Timeline **UI is wired** (2026-07-16, #50, live-verified incl. real chained rewinds and a real worktree fork): the standing per-turn log with the honest edge states, and Rewind/Handoff addressed **k-from-last over the LIVE prompt stack** — the records are append-only (a rewind never truncates them), so the renderer logs each rewind's rolled ordinal range client-side, dims those rows, excludes them from the arithmetic, and keeps turns appended after a rewind live. The fork/retire persistence gaps closed with the 2026-07-16 gap batch: a fork whose spawn-time id discovery missed now **adopts its `claude_session_id`** from the resolved transcript's uuid filename stem — behind an ownership guard, so a shared-cwd fork can never adopt (and permanently pin, or later cold-`--resume`) the source's or a co-located sibling's conversation id — and persists it exactly like a create; **fork lineage** survives archive-on-retire and the resume→retire round-trip; and the **standing log survives retire→resume** — the tmux name (which keys the per-agent launch-config dir holding `turns.jsonl`) rides the resume descriptors and archive rows and is reused on resume (warm rebind when the session is alive, cold create on the same name when dead), with `TmuxBridge.close()` keeping the launch-config dir by default (only the §7.12 true-wipe paths purge it; pre-fix archive rows carry no `tmux_name`, so their resumes honestly fall back to a fresh name). The post-rewind input-box clear is **race-hardened** (2026-07-16, found at that day's e2e): the TUI stages the rewound prompt back into the input box *asynchronously*, so `TmuxBridge.rewind()` no longer fires one blind `Ctrl-U` — it verifies the box reads back empty and re-clears on a short bound, degrading honestly (`input_cleared: false` + a warning on the result, never a raise — the rewind itself succeeded) when the bound exhausts; hermetically pinned in `test_rewind_fork_unit` (empty-box / late-restore re-clear / bound-exhaustion / ghost-suggestion cases), with the live leg riding the existing `test_rewind_handoff_live` rewind proofs. The **rewind anchor is built and live-proven** (2026-07-16/17, the §11 #46 close-out): every turn record now carries `prompt_uuid`/`reply_uuid` — the turn's transcript-entry uuids, lifted at capture with a live-ordering-correct backward scan and a prompt-shape filter (tool results, meta, and sidechain entries excluded; the capture settles until two consecutive lifts agree, so `reply_uuid` anchors the closing entry, not a mid-reply one) — a successful dashboard rewind appends a typed `{type:"rewind", to_prompt_index}` record to the same `turns.jsonl` through the same serialized path, and `GET /sessions/{id}/timeline` replays the interleaved stream server-side into per-row `rolled` flags + merged `rolled_ranges` + the `rewinds` list (still-pending appends merged deduped, so a transient WSL flake can never hide a rewind). The renderer's client-memory rolled marking (`TL_ROLLED`) is deleted — rolled truth comes from the response, so **the marking survives reload and sidecar restart**: proven by `test_rewind_anchor_live` (anchors byte-equal to the real transcript's entries; rolled truth identical across a sidecar kill + warm rebind on the same runtime) and the headed Timeline drive (dimmed rows + the anchor note surviving `page.reload()` twice — single and merged ranges — 40/40 checks, width extremes, full tri-tab/confirm/cancel click coverage). Honest limits, spike-recorded: the engine exposes **no checkpoint id anywhere** (the transcript is append-only — a rewind writes nothing until the next prompt branches via sibling `parentUuid`), and manual-terminal rewinds write no event and stay unmarked — consistent with the Timeline's settled dashboard-turn scope; the persisted uuids make a future transcript chain-walk possible without migration. ⚠ **Today:** the #46 capture window is pollutable in the first seconds after a session restore — a prompt sent before the rebound driver's first transcript poll replays history into `session.events` lands the replay burst inside the new turn's window (wrong anchor/summary, and a replayed assistant entry can satisfy the reply-gated completion) — found by the 2026-07-17 anchor live proof; queue #52 (§11.7).
...
- The **dashboard store** holds exactly three things: **Setups**, **prompt templates**, and the **projects
  index** (`projects.json`, §3.5). Nothing project-specific may live there — with one accepted seam: a session record whose `cwd` resolves to **no** project root (a cwd-less agent) has no project home, so it persists to the app-level `sessions.json` in the same directory — the legacy roster home, kept as the fallback (`save_record` in [`sidecar/runtime_store.py`](../sidecar/runtime_store.py); project rosters always win the aggregate read, and a project-side write removes any app-level copy).
...
├── agents/                    # project-scope agent.md role presets — the Create Role combobox's
│   └── <role>.md              #   Project group (§7.5, §11 #53); front matter read by GET /roles
...
- `agents/` holds the **project-scope role presets** (§7.5, §11 #53) — `agent.md` files whose front matter feeds the Create Role combobox via `GET /roles`; committed with the repo like the rest of the store, so a team's roles travel with it. The catalog's other scope, `~/.claude/agents`, stays 🔌 — surfaced in the same read, never owned or copied.
...
|-----------|:----:|------|--------------------------|---------|
| Agent roster (which agents exist, per project) | 📁 | `state/agents.json` | Team · `agent-node-card`; Agent→Create/Details | matches target — `runtime_store.save_record` routes project-homed records there; cwd-less records fall back to 🏠 `sessions.json` |
| Identity (role/number/name/color/icon) | 📁 | inside `state/agents.json` | everywhere · `identity-badge`, `agent-tile` | matches target |
| Retired identity numbers (never reused) | 📁 | inside `state/agents.json` | — | matches target — persisted on delete; auto-assignment skips them. The in-memory skip set is **process-global** (accepted): `deletion._RETIRED` accumulates every opened project's retired numbers and never resets on project close, so a number retired in one project also gets skipped when auto-assigning in another — number-skip bleed only; the persisted registry stays strictly per-project ([`sidecar/deletion.py`](../sidecar/deletion.py), loaded per project by `state_store.load_project()`) |
...
| Agent role presets (`agent.md` front matter, §7.5/§11 #53) | 📁 | `agents/*.md` | Agent→Create · Role combobox | matches target — read by `GET /roles` (Project scope); the System scope `~/.claude/agents` is 🔌, surfaced never owned |
| Plans (content) | 📁 | `plans/*.md` | Work→Library Plans · `plan-card`, `doc-editor` | plan-mode output redirects there (`plansDirectory` in every materialized settings, §8.5); listing is store-first with a legacy `<root>/<subdir>` fallback, recursive since 2026-07-16 (§7.16) |
...
| Library Assets (media — attachments/citations, §7.14) | 📁 | `assets/<asset_id>/<file>` + `<file>.meta.json` | Work→Library Assets · `asset-card` | matches target — data endpoints + the tab UI/Attach flow wired (#50, 2026-07-16); delete ships (`DELETE /library/assets/{asset_id}`, 2026-07-16) with Remove enabled behind the confirm |
| Inbox items | 📁 | `state/inbox.json` | Feed→Inbox · `*-inbox-card` | matches target (§8.3) |
| Links | 📁 | `state/links.json` | Work→Links + the Team-panel Link drawer · `link-drawer`, `link-list`, `link-edges` | matches target (§8.3) |
...
| Message feed / cap metrics / console / subagents / run-strip / marquee | ⚡ | — (derived, §8.3) | Feed / Team / Agent→Console | matches target |
...
6. **The overlay-index principle.** Anything the dashboard adds *about* transcript content is keyed to the
   ...
   re-join losslessly on replay. The lossless re-join holds for **transcript-anchored** events only (accepted): a SYNTHESIZED event has no anchor, so `stamp()` mints it a seq-based id (`{agent_id}:s:seq{n}`) and a routing row keyed by that id has no transcript entry to re-join to on a later replay — synthesized events are sidecar-born notices, not transcript content, so nothing durable is lost.
...
transcripts into the feed. The active-project chip appears; the panes fill. The picker/chip UI and the picker-first startup are wired (§11 #50, 2026-07-16): with no open project, the renderer's first `/projects` read steps into Settings → Projects with the last-used row preselected — startup never silently auto-loads a project. The sidecar matches (2026-07-16 gap batch): **startup restores nothing by default** — `POST /projects/open` is the restore point, running `reconnect_sessions(project_key=…)` for exactly the opened project — with `AWL_STARTUP_RESTORE=all` as the restore-everything-at-boot escape hatch for tests and one-project setups. (A fork's git-worktree cwd also no longer self-registers into the picker — the §3.5 `registered` flag.)

### 9.2 Create an agent
`POST /sessions` → the sidecar assigns identity and coerces an unspecified model to the product default,
Opus (§7.5) → the `bridge` driver `start()` →
...
- **Warm** (sidecar restarted; tmux/WSL still running): rebind to the live session. This works today —
  ...
  held them.
- **Cold** (reboot / WSL shutdown; tmux gone): relaunch the agent with `claude --resume <claude_session_id>` in its cwd — the same conversation, rebuilt from the transcript, continuing on the **same** session id and the same `<id>.jsonl` (plain `--resume` never forks; a fork takes the explicit `--fork-session` flag — live-proven on CC 2.1.202 in [`tests/test_cold_restore_live.py`](../tests/test_cold_restore_live.py)). The path is wired end to end: `create(resume_session_id=…)` launches `--resume <id>` with no `--session-id` ([`bridge/bridge.py`](../bridge/bridge.py)), and `reconnect_sessions()` in [`sidecar/main.py`](../sidecar/main.py) cold-restores every dead-tmux record that carries a `claude_session_id` (a full create — launch config, hooks, retention pin all apply; the resumed transcript replays into the feed via the driver poll). A **zero-turn** agent is the one exception (2026-07-17, live-proven): Claude Code writes the `<id>.jsonl` only on the **first turn**, so an agent spawned and retired without ever being prompted has no transcript, and `claude --resume <id>` dies at launch ("No conversation found") — the driver now **probes transcript existence** before a cold restore ([`sidecar/drivers/bridge.py`](../sidecar/drivers/bridge.py)) and relaunches such an agent **fresh, pinned to the same session id** (`--session-id <id>`, exactly the original launch — an empty conversation on the same id IS the same agent returning); an unreadable probe fails open to `--resume`, so a transient WSL hiccup can never discard a real conversation. Only a record with **no** claude id (no way back to the conversation) is pruned. Graceful-degrade fallback if cold-restore proves shaky in practice: restore all *data* and let agents be re-resumed manually. ⚠ **Today:** the rarer warm-path twin is queued (#57, §11.3) — `TmuxBridge.resume` still falls through to a bare `--resume` without the probe when tmux dies between its alive-check and the rebind, the same zero-turn edge on the warm path.
...
record, tmux session, transcripts incl. subagents, the per-agent launch-config dir with its Timeline/statusline stores, the agent's rows in the project `state/` files) →
...
- **Decision (research settled, [`native-claude-code-coordination-report-2026-07-02.md`](../dev/notes/research/native-claude-code-coordination-report-2026-07-02.md)):** keep the sidecar's custom spine (inbox / links / scratchpad) **canonical**; adopt native primitives only narrowly, as observability enrichments or optional run modes. Findings: `Task` was renamed `Agent` in v2.1.63 (the parser-compat audit closed 2026-07-10 — the parser was already dual-name everywhere, and an end-to-end pinning test was added; §11 #36); `TodoWrite` is disabled-by-default (v2.1.142; not the adoption target); `SendMessage` is scoped to native subagent/team graphs and **cannot** bus across independently spawned tmux processes (confirms links stay custom); agent-teams are experimental — one team/session, no nesting, no resume.
...
**6. Docs-in-agent-context — the automatic retrieval layer** *(→ §7.16, §11 #44)* — ⏸ **future layer; v1 built 2026-07-16 (§11 #44)**
- The v1 mechanism is settled and built (curated Library home + per-agent doc attachment at launch, §11 #44, 2026-07-16). What stays open is only the *automatic* layer: how relevant, current docs are selected and refreshed per agent role — context injection, an MCP docs server, and hook-pushed digests are the candidate mechanisms.
...
|--------|-------------------|------------|
| §7.13 | expanded-console layout at common window widths — the step-into keeps the right column + a 270px commands rail, leaving ~60 terminal columns at 1440px | #54 (§11.5) |
| §7.16 | CC 2.1.212's parked plan-review box reads `idle`, not `permission_prompt` — a user-queued prompt would flush into the parked box | #51 (§11.4) |
| §7.19 | #46 capture window pollutable in the first seconds after a restore — the history-replay burst can mis-anchor the first post-restore turn and satisfy the reply gate | #52 (§11.7) |
| §9.9 | warm-path zero-turn resume race — `TmuxBridge.resume` skips the transcript probe when tmux dies between alive-check and rebind | #57 (§11.3) |
...
### 11.3 Agent control & lifecycle (#12–20, #53, #57)
...
53. *(built 2026-07-17 — `GET /roles` + the Create Role preset loader (§7.5, DESIGN.md ND 12): the two-scope `agent.md` catalog — System `~/.claude/agents` via the `AWL_SYSTEM_AGENTS_DIR`-overridable resolver, Project `<project>/.awl-cc-dash/agents/` via the new `storage.agents_dir` (§8.2 subdir) — parsed by [`sidecar/roles.py`](../sidecar/roles.py)'s hand parser (fences/scalars/inline+block lists/quoted scalars/commented-key skip, snake_case wire), with the named-colour→AG_COLORS-hex map (`NAMED_COLOR_TO_HEX`, mirrored by behavior.js `AG_FROM_NAMED`); the Create combobox prefills from a pick — colour included, icon never (the per-agent differentiator); prefill-only v1 — the bridge ignores `agent_type`, only the sdk driver maps `--agent`. Live-proven in the 2026-07-17 drive. See DEVLOG.)*
57. **Warm-path zero-turn resume race** — the cold-restore path probes transcript existence before `--resume` (§9.9, built 2026-07-17), but `TmuxBridge.resume` still falls through to a bare `create(resume_session_id=…)` when the tmux session dies between its alive-check and the rebind — a transcript-less (zero-turn) id would ride `--resume` and die at launch, the same edge the cold path now guards (§9.9 ⚠, found at the 2026-07-17 live drive). Fix: route the warm fall-through through the same probe.

### 11.4 Coordination spine, hooks & inbox (#21–28, #51)
...
22. *(built 2026-07-10 — `POST /sessions/{id}/plan/verdict` (approve = the proven `keys()` Enter; revise = Escape + queued feedback) + `PUT /library/document` edit-in-place. **Revise leg live-proven 2026-07-17** (`test_plan_verdict_revise_then_approve_live`, sidecar-owned real plan-mode agent, CC 2.1.212): Escape keeps plan mode, the queued feedback flushes and revises, approve executes — the ⚠ assumed marker is cleared; the 2.1.212 menu drift + the parked-box `idle` status watch item are recorded at §7.16. See DEVLOG.)*
...
51. **Plan-box status classification (CC 2.1.212)** — teach the bridge's screen-state parser the parked plan-review box: on 2.1.212 the box no longer classifies as `permission_prompt` (its trailing hint lines outrun `parse_permission_prompt`'s bottom-anchor slack), so `status` reads `idle` while the box is parked and a user-queued prompt would flush into it (§7.16 ⚠, found by the 2026-07-17 #22 revise live proof). Fix is a parser case (recognize the plan-box wording the live test pins) + a queue-flush gate on it; the #22 endpoint itself is unaffected.

### 11.5 Readouts, console & cost (#29–36, #54)
...
54. **Expanded-console layout at common widths** — the Console's step-into expand preserves the right column plus a 270px commands rail, so at a 1440px window the terminal itself gets only ~60 columns — generous at 2560px (the live-proven 211×58 fill) but cramped at the layout floor (§7.13 ⚠, found at the 2026-07-17 live drive). Direction: a full-window expand or a collapsible commands rail.

### 11.6 Frontend build (#37–41, #50, #55–56, #58–59)

37. *(built 2026-07-15 — the visible renderer was rebuilt fresh from `design/` (recovered from the stranded lane, merged, verified headed) and WIRED to the backends: 25-colour tokens, marquee, the live xterm **Console** (#29, e2e-proven streaming a real tmux screen), context breakdown (#30), response presets (#39), randomize (#40), authors provenance (#41), degraded-mode + narrow-width (#38); `api.ts` carried as the preserved contract. Residual UI at rebuild time, since closed: the **Timeline** surface landed with #50 + the #46 anchor (2026-07-16); per-agent **cost-on-cards** was decided OUT by the operator (2026-07-15, §7.15 — the cards stay Usage-only); the **resume/archive-roster** (#17/#18) and **Bypass/Auto** gating (#13) UIs landed with #50 (2026-07-16). See DEVLOG.)*
...
50. *(built 2026-07-16 — both halves. **Design half** (37 headed screenshots + the Timeline surface's 41/41 checks): launch-arm gating, Past tab, Review card, Import control, subagent activity dot, Timeline surface; Projects surfaces verified in the mockup. **Renderer wiring half** (same day; headed-verified against a LIVE sidecar with real tab-less agents, then integrator-re-driven post-review — real chained rewinds, a real worktree fork, the review-beat hold, the attach guards): Projects chip/picker/close-confirm + picker-first startup + the styled Electron close dialog over IPC (native fallback, with reload/crash rescue so an un-answered dialog can never wedge the close path); the #13 launch-arm UI (arm-box + un-armed segments absent from the Details ring, honest-limit notes for BOTH arms); the #17/#18 Past tab (resume + archive incl. true-delete confirm); the #23 Review card (resolved card held in place for the 1.2 s read beat independent of poll phase); the #28 Import drawer; the §7.17 activity dot + subagent tree (client `normalizeSubs` blend repair); the §7.19 Timeline with Rewind/Handoff addressed k-from-last over the LIVE prompt stack (client rolled-range marking — chained rewinds and post-rewind turns stay correct); §7.14/§7.16 Attach/Citations/Assets (256 MB pre-check before encode, send-4xx details surfaced verbatim, thumbnail IPC built in main/preload). **Residuals** — verification: the interactive Electron-window close-dialog drive and the thumbnail-IPC Electron-renderer drive ride the phase-3 e2e. The backend gaps the wiring surfaced were **fixed 2026-07-16 (the phase-3 gap batch, integrator-hardened)**: the derived `armed_modes` read + `arm_bypass` arm-without-activate (persisted roster AND archive) + the live `auto` set-mode spelling (§7.11); the sidecar-side subagents blend (§7.17); died-at stamps on past rows, with stale stamps cleared on restore; asset delete + the enabled Remove (§7.16); picker-first sidecar startup + the §3.5 `registered` flag (§9.1); and the §7.19 fork/retire persistence set — fork id adoption behind an ownership guard (a shared-cwd fork can never adopt the source's/a sibling's conversation id), lineage through archive-on-retire and resume→retire, and Timeline-store survival across retire→resume (tmux-name reuse + `TmuxBridge.close()` keeping the launch-config dir; only true-wipe paths purge). The #46 rewind-anchor residual **closed 2026-07-16/17** (§7.19 — server-persisted anchors + rewind events + replay; the client rolled-range marking is retired for server truth, reload-survival headed-proven). See DEVLOG.)*
55. **Window width-floor decision** — Electron `minWidth` is 1180 (the usable-with-inner-scroll floor) while the no-scroll layout floor is 1440 (pinned 400 + 340 middle-min + pinned 700); the #38 horizontal-scroll guard covers the band between (found at the 2026-07-17 live drive). Decide: raise `minWidth` to 1440, or reflow the pinned columns so the layout genuinely fits below it.
56. *(built 2026-07-18 — the manual icon picker serves the FULL shipped set (§7.5): a new `GET /assets/agent-icons` listing endpoint returns every stem on disk (`identity.AG_ICONS`, 167 — each servable through the recolor route), and the renderer picker feeds from it (module-cached fetch, sprite `<use>` fast-path for embedded names, recolor-endpoint `<img>` tiles for the rest, degrade to the sprite-embedded 50 on fetch failure — never an empty picker). Riding the same seam, the icon-rendering fixes: the sprite alias strips the curated pool's `__author` suffixes and the missing-sprite fallback paints with the AGENT'S colour (the invisible white-on-white tile that hid 11 curated stems — dead), and the recolor endpoint gained `layer=glyph` (bg knocked to `fill:none`, glyph tinted) so graph-node watermarks render design-correct polarity for all 167 icons. Hermetically pinned (`test_agent_icons_unit.py`, 22 cases) and live-proven in the 2026-07-20 drive: the picker counted exactly 167 tiles, and a non-sprite icon rendered agent-coloured on the roster tile and glyph-polarity on the graph watermark. See DEVLOG.)*
58. **Live-drive cosmetics batch (2026-07-17)** — small visual defects from the live drive, batched: the agent-node-card watermark icon overflows its card by ~8px, and the narrow-mid floor (the 340px middle column) clips — the Library tabset, the Plans counter overlapping, and the Create popups rendering under `pcard-foot`.
59. *(built 2026-07-18 — the rest of the [NU] NEXT UP run (the dev/notes/TODO.md queue), consolidated; live-proven in the 2026-07-20 headed drive against a fresh sidecar + real warm-rebound agents, width extremes 1180/2560 included: the §4.2 panel renames (**Team** / **Feed**, zero old-name strings in the live DOM); the Past tab → the Team panel's 416px **Past drawer**, mutually exclusive with Link Config — same resume-picker + archive-roster content against the unchanged #17/#18 endpoints, proven through a live retire→resume→true-delete round-trip; the Library open plan/doc card **flex-fills** its panel (280px floor, list scrolls, the comment popout never crushed — DESIGN.md owns the visual detail); the §7.13 **console attach reuse** (expand = zero re-attach POST, 133 ms live-measured, invalidated on socket death/retire/delete/resume); and the §7.5 **Opus model default** (no Inherit anywhere, `DEFAULT_MODEL` coercion pinned by `test_sidecar_unit.py::TestModelDefault`, the five seeded role presets pinned to `model: fable`, a UI-created zero-turn probe persisting `model: "opus"`). See DEVLOG.)*

### 11.7 Support systems (#42–49, #52)
...
46. *(built 2026-07-16 — one thin Timeline record per dashboard turn at the exactly-once completion gate (bridge reply-gated run→idle + SDK `result`): settings-at-turn joined from the #31 statusline model + the §7.4 arbiter mode + the tracked effort/thinking levers (set-effort now validated + idle-gated — `/effort` has no read-back), plus a one-line summary off the reply's leading line (the #39 lean); captures serialized per session, persisted to per-agent `turns.jsonl` with in-order retry, served by `GET /sessions/{id}/timeline` (ordinals re-minted in stored order). Rewind doesn't truncate the records — the row→rewind/fork mapping landed with #50 (k-from-last over the LIVE stack). **The anchor closed 2026-07-16/17**: records carry `prompt_uuid`/`reply_uuid` transcript anchors, rewinds persist typed records, the read surface replays rolled state server-side, and the renderer consumes server truth — live-proven incl. sidecar-restart survival (`test_rewind_anchor_live`) and headed reload survival (§7.19). See DEVLOG.)*
...
52. **Post-restore capture-window guard** — the first dashboard-initiated turn after a session restore is pollutable: a prompt sent before the rebound driver's first transcript poll replays history into `session.events` lands the replay burst inside the new turn's #46 capture window — wrong `prompt_uuid`/summary (observed live: a DELTA turn anchored to the replayed ALPHA prompt), and a replayed assistant entry can satisfy `_saw_reply_since_send`, defeating the reply-gated exactly-once completion (§7.19 ⚠, found by the 2026-07-17 #46 anchor live proof; evidence in `tests/log/rewind_anchor_findings_*`). Direction: gate the first post-restore queue flush on the initial replay completing, or exclude pre-existing transcript entries from the anchor/summary window and the reply gate. Scope: the ~1-5 s window after warm/cold restore only; normal operation unaffected.
...
| [`frontend/src/renderer/store.tsx`](../frontend/src/renderer/store.tsx) | The renderer's shared store — SSE bus, §4.3 polling, degraded mode (§4.2) |
...
| [`sidecar/runtime_store.py`](../sidecar/runtime_store.py) · `state_store.py` · `runstate.py` · `identity.py` · `deletion.py` · `storage.py` | Restart-surviving session records · per-project write-through state store (§8.3) · hook run-state arbiter (§7.4) · identity assignment · hard-delete/tombstone · project-store paths |
| `links.py` · `scratchpad.py` · `watermark.py` · `library.py` · `attachments.py` · `roles.py` · `templates_store.py` · `prompt_library.py` · `response_presets.py` · `console_catalog.py` · `checklist.py` · `marquee.py` · `subagents_naming.py` · `timeline.py` · `handoff.py` · `import_context.py` · `changelog.py` · `system_check.py` · `settings_io.py` · `utility_llm.py` | Coordination-spine feature modules: linking · scratchpad · read-watermarks · library · attachment/citation asset materialization (§7.14) · the two-scope `agent.md` role-preset catalog (§7.5, §11 #53) · templates · scope-aware prompt/UI-text library (§11 #45) · Response presets (§11 #39) · console catalog · checklist parse · marquee tail · subagent naming/blend · per-turn Timeline capture (§7.19) · handoff-report generation (§11 #16) · external-session import (§11 #28) · change-log rendering (§11 #48) · system-check probes (§11 #49) · settings read/write · utility-LLM passes |
| [`bridge/bridge.py`](../bridge/bridge.py) · `transcript.py` · `paths.py` · `mcp.py` · `registry.py` · `cli.py`/`__main__.py` | tmux/WSL2 control · JSONL transcript resolution · path/net translation · MCP sync · Settings reads · the `python -m bridge` CLI |
