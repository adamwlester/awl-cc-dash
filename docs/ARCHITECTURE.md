...
The renderer is the #37 rebuild (§4.4) — built fresh from the `design/` system, its DOM and CSS mirroring `mockup.html`. [`main.tsx`](../frontend/src/renderer/main.tsx) mounts [`App.tsx`](../frontend/src/renderer/App.tsx), a thin shell reproducing the design frame — title bar · three resizable columns (Agent | Team Graph / Library | Team Feed / Prompt) · status footer — plus the Settings and Console step-into views. The pieces, all under `frontend/src/renderer/`:

| Component | File | Role |
|-----------|------|------|
| `AgentPanel` | `components/AgentPanel.tsx` | Left pane — Details / Create / Console tabs for the focused agent (identity edits, the live mode/model/effort/fast/thinking levers, Retire/Delete, the #13 arm-gated mode ring) |
| `TeamGraph` | `components/TeamGraph.tsx` | Agent cards — identity row, status/model badges, Ctx/Turns bars, run-strip, marquee, subagent strip |
| `TeamFeed` | `components/TeamFeed.tsx` | Right-top — Transcript · Scratch · Log · Inbox over the shared From/To filter accordion |
...
| `LinkDrawer` | `components/LinkDrawer.tsx` | The Link Config drawer, opening against the right column's edge |
...
|--------|-------------------|------------|
| §7.5 | the renderer's manual icon picker offers only the 50 sprite-embedded icons — the recolor endpoint already serves the full shipped 167 | #56 (§11.6) |
...
### 11.6 Frontend build (#37–41, #50, #55–56, #58)
...
56. **Icon picker offers 50 of the 167 shipped icons** — the renderer's manual picker reads the 50 symbols embedded in [`sprite.ts`](../frontend/src/renderer/design/sprite.ts) (extracted from mockup.html's embedded set), while `assets/icons/agents/` ships 167 and the recolor endpoint (`GET /assets/agent-icons/{name}?color=`) already serves them all (§7.5 ⚠, found at the 2026-07-17 live drive). Direction: feed the picker from the endpoint's full set.
