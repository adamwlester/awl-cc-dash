...
The renderer **rebuild is done** (§11 #37, built 2026-07-15): the visible renderer was rebuilt fresh from `design/`, wired to the built backends, and e2e-proven (25 colours, marquee, the live xterm Console). The `tests/ui/` slice exists (built in the 2026-07-02 spike batch). ⚠ **Today:** the residual UI rides §11 #50 — surfaces the mockup doesn't carry yet (Projects, Bypass/Auto gating, resume picker, archive roster, Review card, Import) plus the Timeline (its #46 capture is built 2026-07-16; the surface rides #50).
...
| §7.19 | Rewind/fork endpoints + the #46 capture + the Timeline mockup surface are built; the renderer wiring (and the row→rewind-checkpoint anchoring) remains | #50 |
...
32. *(built 2026-07-14 — per-agent `/cost` scrape behind on-demand `GET /sessions/{id}/cost` (idle-gated, `null` on no panel, deliberately never in the poll loop); live acceptance green on CC 2.1.206; the on-card display rides #37; see DEVLOG)*
