...
`GET /assets/agent-icons/{name}?color=`. The Create panel's randomize affordance draws from the **curated name pool** shipped at [`assets/names/agent-names.json`](../assets/names/agent-names.json) — 179 one-word, 3–5-letter, lowercase names, validated to double safely as git commit-author names (§11 #19) — with user-typed names always available; the randomize/auto-name draw is wired (§11 #40, built 2026-07-15). The Create panel's **role number auto-fills**: the No. field pre-fills the next value in that role's sequence (e.g. a second `researcher` pre-fills `02`) but **stays editable** — runtime behavior with nothing to draw in the static mockup (recorded 2026-07-08 from the design lane's IN-2 note). Identity editing + name registration are wired (§11 #14, built 2026-07-10): `POST /sessions/{id}/identity`
...
### 11.7 Platform, hygiene & support (#42–49, #52)
...
| `links.py` · `scratchpad.py` · `watermark.py` · `library.py` · `attachments.py` · `templates_store.py` · `prompt_library.py` · `response_presets.py` · `console_catalog.py` · `checklist.py` · `marquee.py` · `subagents_naming.py` · `timeline.py` · `handoff.py` · `import_context.py` · `changelog.py` · `system_check.py` · `settings_io.py` · `utility_llm.py` | Coordination-spine feature modules: linking · scratchpad · read-watermarks · library · attachment/citation asset materialization (§7.14) · templates · scope-aware prompt/UI-text library (§11 #45) · Response presets (§11 #39) · console catalog · checklist parse · marquee tail · subagent naming/blend · per-turn Timeline capture (§7.19) · handoff-report generation (§11 #16) · external-session import (§11 #28) · change-log rendering (§11 #48) · system-check probes (§11 #49) · settings read/write · utility-LLM passes |
