...
| **Console** | `GET /console/catalog` · `POST /sessions/{id}/console/run` · `POST /sessions/{id}/console/{attach,detach}` (the live streaming terminal — geometry-pinned `ttyd`/WebSocket attach, §7.13; the xterm.js renderer that consumes it rides the rebuild, §11 #37) |
...
⚠ **Today:** the **backend streaming attach is built** (§11 #29 backend, built 2026-07-14) — `POST /sessions/{id}/console/{attach,detach}` pins the pane geometry first, then starts a writable `ttyd` on the tmux session and returns the `{ws_url, url, port, reused}` the renderer consumes (WS bytes flow from Windows over `localhost`, one attach per session, detach idempotent; live-proven `test_console_attach_endpoint_live`, CC 2.1.206). What **remains** is the **xterm.js-class renderer** in the rebuilt Console (attach-on-open / detach-on-close, bounded scrollback catch-up, keystroke passthrough — and it must always drain its WS, since a stalled reader backpressures the pty), which rides the renderer rebuild (§11 #37); the React Console is still stubbed in places today. Interception stays on the JSONL transcript, never the stream.
...
29. **Console xterm renderer** *(→ §7.13; **HIGH** within #37; backend attach built 2026-07-14 — see DEVLOG)* — the **backend streaming attach shipped**: `POST /sessions/{id}/console/{attach,detach}` pins the pane geometry (`window-size manual`) first, then starts a writable `ttyd` on the tmux session and returns `{ws_url, url, port, reused}` (one attach per session, detach idempotent; live-proven `test_console_attach_endpoint_live`, CC 2.1.206). **Remaining:** the xterm.js-class renderer in the rebuilt Console (attach-on-open / detach-on-close, bounded scrollback catch-up, keystroke passthrough, always-drain the WS) — rides the renderer rebuild #37. Interception stays on the JSONL transcript, never the stream. Where: rebuilt renderer.
