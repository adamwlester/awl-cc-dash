...
| `test_archive_unit.py` | Agent archive (§7.12/§8.4, §11 #18) — `deletion.build_archive_record` (LIGHT records, reserved lineage, transcript referenced in place), Retire→archive round-trip, Delete-never-archives, `GET/DELETE /archive` endpoints | ~15 |
| `test_resume_past_unit.py` | Load past agents / on-demand resume (§9.9, §11 #17) — `GET /sessions/past` enumeration (roster + archive, `resumable`/`died_at`), `POST /sessions/resume` by id/name/archive_id riding the #8 cold path, un-retire-on-resume, warm-rebind vs cold-create on the persisted tmux name, lineage/arm_bypass carry | ~31 |
