...
|------|-------------------------------|:-----:|
| `test_bridge_unit.py` | `bridge/` pkg (screen-state detection, context-usage & turn derivation, transcript parsing, the `create()`/`resume()` launch argv incl. `--name` display-name registration §7.5 and the §7.5 Opus-default emit (`create(model="opus")` lands `--model opus` in the argv), and the §11 #12 live-control parsers — mode indicator / thinking modal / fast panel — with the bounded Shift+Tab cycle + read-first toggles via scripted fakes) + `sidecar/drivers/base.py` + `sidecar/drivers/bridge.py` (non-live paths, incl. `set_display_name` → `/rename`) + `sidecar/runtime_store.py` | ~156 |
| `test_sidecar_unit.py` | `sidecar/main.py` endpoints (incl. the Library sidecar-store endpoints: reviews migration+aggregate, document create/delete/rename, comments + their `.awl-cc-dash/` scope guards; the link endpoints' §7.6 one-relationship contract + per-relationship trigger defaults; the §7.5 identity-edit endpoint with retired-number refusal + rename wiring; the §11 #12 mode/fast/thinking control endpoints with their honest 409/400 mapping) + driver wiring + `identity.py` + hookbus/links/eventbus integration, the reply-to relay, and the shared-context fire / piggyback delivery — plus the 2026-07-17 pins: `CreateSessionRequest`/`DriverConfig` default `permission_mode` is `bypassPermissions` (with the `armed_modes` derivation), and a blank/absent create name auto-draws from the curated pool (live-name exclusion; explicit names pass through) — and `TestModelDefault` (the §7.5 Opus default, 2026-07-18): `DEFAULT_MODEL = "opus"`, missing/empty/any-case-`inherit`/explicit-JSON-null models coerced to it at create (never a 422), the coerced model reaching the driver, explicit models passing verbatim | ~140 |
...
| `test_library_unit.py` | `library.py` (doc/plan render incl. the §7.16 listing scope — flat default vs `recursive=True` store walks with `rel_path`, cycle-safe against junction/symlink loops, `exclude_top` for the `docs/prompts/` §11 #45 copy; per-doc `.meta.json` sidecars §8.5 — review/comments/anchors/provenance, atomic writes, pair-rename, orphan re-link, legacy `plan-reviews.json` migration, recursive reviews aggregation + nested bare-filename resolution, store-scoped create/delete guards) | ~94 |
| `test_attachments_unit.py` | Attachment/citation path materialization (§7.14, Option A — ex-§10 #1) — `attachments.py`: base64/source-path ingest into `<project>/.awl-cc-dash/assets/<id>/<name>` with the `<name>.meta.json` sidecar (sha256/size/mime/created/provenance + the §7.14 citation anchor), filename sanitization across the WSL boundary (incl. reserved device names) + the pre-read size cap, automatic write-leg selection (`store_kind`), the traversal-safe serve gate (sidecars unaddressable case-insensitively), per-receiver renderings (agent WSL path — both UNC spellings — / renderer HTTP URL), the attributed message-attachments block (prompt-library lead + project override), the `POST/GET /library/assets` + `GET /assets/{id}/{filename}` endpoints' honest degrades, the `/sessions/{id}/send` attachments wiring (all dispositions incl. inject; unknown id → 400), and the §7.16 recursive `GET /library/documents` store listing incl. the `docs/prompts/` exclusion | ~69 |
| `test_agent_icons_unit.py` | The agent-icon asset routes (§7.5, §11 #56) — `GET /assets/agent-icons/{name}`: the exactly-once background recolor, the invalid-`color` SVG-injection guard, the 3 `layer=glyph` polarity pins (bg knocked out + glyph tinted; no-valid-color and unknown-layer fallbacks), traversal/unknown-stem 404s, and the 11 formerly sprite-mismatched curated stems (the "fnord has no icon" bug) all servable — plus `GET /assets/agent-icons`, the full-set listing feeding the manual picker (sorted `identity.AG_ICONS`, every stem an existing `.svg` on disk) | ~22 |
...
| `test_console_attach_unit.py` | Console streaming attach, backend half (§7.13, §11 #29) — `pick_console_port`, `TmuxBridge.console_attach/console_detach` via scripted fakes (geometry pinned BEFORE ttyd, writable `-W` invocation, one-attach-per-session reuse, detach leaves the pin, close() detaches first), and the `POST /sessions/{id}/console/{attach,detach}` endpoint wiring — plus `TestConsoleResize`, the bridge-level half of the §7.13 sanctioned geometry write (`TmuxBridge.console_resize`: exact `resize-window` command shape, internal 60–500×15–200 clamps, never-raises `{ok: False, reason}` degrades) | ~27 |
| `test_console_resize_unit.py` | Console resize endpoint, sidecar half (§7.13 geometry seam) — `POST /sessions/{id}/console/resize` → `TmuxBridge.console_resize` wiring: belt-and-braces clamps (cols ∈ [60, 500], rows ∈ [15, 200] BEFORE the bridge call), the applied-values echo, and the honest error mapping (bridge `busy` → 409, other bridge failure → 400, unknown session → 404, non-bridge driver → 400, raising bridge → 500) | ~8 |
| `test_readouts_unit.py` | The STAGE-3 readout surfaces (§11 #30/#31/#32) — `parse_context_output` (screen + markdown-table shapes) + `compact_history` and the breakdown endpoint (409 busy / 400 no-capability / fetched_at); the statusLine capture settings + `get_statusline_snapshot` tail-read + `/context`'s `per_turn` field; `parse_cost_output` (the `/cost` per-session panel — never any bare `$`), the driver's idle-gated `get_cost`, and the on-demand `GET /sessions/{id}/cost` endpoint; the §11 #46 Timeline contract — `build_record` anchors (`prompt_uuid`/`reply_uuid`, null-safe), `turn_anchors` (live-ordering backward scan, `_prompt_like` tool-result/meta/sidechain exclusion), the capture settle loop, `build_rewind_record` + the serialized rewind-event append (success-only), and `replay_timeline` (ordinal minting over turn records, k-clamp, chained rewinds, typeless old lines, pending-record merge dedupe, `rolled`/`rolled_ranges`/`rewinds` response) | ~95 |
...
| `test_storage_unit.py` | `storage.py` (on-disk store; canonical-root aliases incl. the `\\wsl$\` → `\\wsl.localhost\` fold + both UNC spellings stripping to native WSL paths) | ~17 |
...
| `test_state_store_unit.py` | `state_store.py` (the per-project `state/` layer §8.3 — agents/inbox/links/bookmarks JSON + routing.jsonl, atomic write-replace, `schema_version`, write-through persist hooks, merge-shaped concurrent writes, the archive table CRUD) | ~27 |
| `test_archive_unit.py` | Agent archive (§7.12/§8.4, §11 #18) — `deletion.build_archive_record` (LIGHT records, reserved lineage, transcript referenced in place), Retire→archive round-trip, Delete-never-archives, `GET/DELETE /archive` endpoints — plus the 2026-07-17 gating: `save_archive_record` raises on a no-op, retire teardown gated on the archive landing (failure → the overridden `stop()` keeps the roster row as a resumable Past entry; `record_kept`/`archive_error`/`archive_skipped` response fields), idempotent re-archive (a standing same-session row is replaced, never duplicated) and the `/sessions/past` per-session dedup | ~25 |
| `test_resume_past_unit.py` | Load past agents / on-demand resume (§9.9, §11 #17) — `GET /sessions/past` enumeration (roster + archive, `resumable`/`died_at`), `POST /sessions/resume` by id/name/archive_id riding the #8 cold path, un-retire-on-resume, warm-rebind vs cold-create on the persisted tmux name, lineage/arm_bypass carry — plus the 2026-07-17 zero-turn cold-restore pins (§9.9): the transcript-existence probe routes a provably transcript-less id to a FRESH launch pinned to the same session id, `--resume` when the transcript exists, fail-open to `--resume` on an unreadable probe | ~35 |
| `test_projects_system_unit.py` | The Projects system surface (§3.1–§3.5, §11 #26) + System identity/faults (§7.2, §11 #27) — `GET /projects` + register/open/close, the `registered` flag (fork worktrees never self-register), record-keeping stop, coalesced System cards + the liveness probe | ~20 |
| `test_attached_docs_unit.py` | Docs in agent context (§7.16, §11 #44) — `attached_docs` through CreateSessionRequest → DriverConfig → roster persist, the WSL-path consult preamble composed with the #39 preset via `--append-system-prompt`, best-effort skip of unresolvable refs | ~37 |
| `test_handoff_artifact_unit.py` | Handoff artifacts (§7.19, §11 #16) — `handoff.py` transcript-prefix summarization plumbing (utility LLM stubbed), the 3-section markdown doc stored with provenance, the fork `handoff` flag + standalone endpoint's honest errors | ~17 |
| `test_name_pool_unit.py` | Name-pool wiring (§7.5, §11 #40) — `identity._load_name_pool`/`draw_name` (case-insensitive exclude, exhaustion fallback, injectable rng) + `GET /identity/random-name` | ~13 |
| `test_roles_unit.py` | The two-scope `agent.md` role catalog (§7.5, §11 #53 — DESIGN.md ND 12) — `roles.py` + `GET /roles?cwd=`: the hand parser (fences, scalars, inline/block lists at any indent, quoted scalars, commented-key skip, no-front-matter files, filename-stem name fallback), the snake_case wire from the camelCase file keys (`permissionMode`/`maxTurns`), `color` + `color_hex` (named→AG_COLORS map, `#rrggbb` passthrough, unknown → null), the LIGHT payload (no markdown body) with per-scope name sort + unreadable-file skip, and the endpoint's two scopes (`AWL_SYSTEM_AGENTS_DIR` override; no-project `dir: null` honest degrade) | ~22 |
| `test_authors_provenance_unit.py` | Authors provenance (§8.5, §11 #41) — `library.doc_provenance` riding `GET /library/documents`/`document` additively | ~9 |
| `test_response_presets_unit.py` | Response-format presets (§7.14, §11 #39) — the 6-entry catalog (`tldr_table` etc.), per-agent persist, `--append-system-prompt` injection, prompt-library override resolution with the in-code default | ~18 |
| `test_support_tools_unit.py` | The support set (§11 #47–#49) — `POST /sessions/{id}/git` (status/diff/commit with explicit `git_env` identity injection, honest 400/409), `changelog.py` + the refresh endpoint (AI-author query → rendered change-log doc), `system_check.py` + `GET /system-check` aggregation (probes mocked) | ~69 |
...
| `test_console_attach_endpoint_live.py` | The §11 #29 **acceptance**: `POST /sessions/{id}/console/attach` returns a live ws_url a real Windows WebSocket streams bytes from; a turn completes (and the pane geometry holds) UNDER the attached, draining viewer — coexistence; re-attach reuses; detach closes the port — plus the §7.13 **resize round-trip leg** (live-proven 2026-07-17): `POST /sessions/{id}/console/resize` lands `resize-window` geometry on the live pinned pane (tmux read-back matches the applied values) with the scraper still classifying after (verdict in `log/console_attach_endpoint_findings_latest.txt`; skips without ttyd) | ~1 |
...
| `test_attachments_live.py` | The §10 #1 **two proof legs**: (A) the WSL-native write path — a real WSL-internal project store ingested via `wsl.exe … cat > tmp && mv` with in-distro sha256 verify, `wslpath -w` round-trips on spaces/unicode names, the installed-distro UNC form, and a WSL-internal `source_path`; (B) the asset-endpoint byte render through a real in-process app client (FastAPI TestClient: POST→GET byte-identical + content-type, traversal 404s, agent-icons route precedence) plus a real-uvicorn ephemeral-port spot-check (findings in `log/attachments_findings_latest.txt`) | ~8 |
...
| `test_plan_decision_hooks_live.py` | …surface `ExitPlanMode`/`AskUserQuestion` as inbox cards, then resume the agent? | ✅ FEASIBLE — and the §11 #22 verdict endpoint is live-proven end to end (2026-07-17, `test_plan_verdict_revise_then_approve_live`, sidecar-owned session on CC 2.1.212): revise's Escape keeps plan mode + the queued feedback flushes and revises; approve executes the plan |
| `test_rewind_anchor_live.py` | …do the §11 #46 anchors match the real transcript, does a rewind persist, and does rolled truth survive a sidecar restart? | ✅ PROVEN (2026-07-17): `prompt_uuid`/`reply_uuid` byte-equal to the real JSONL entries; the typed rewind record lands in `turns.jsonl`; rolled flags/ranges identical across a sidecar kill + warm rebind; the post-rewind branch turn anchors to the new sibling entry (also caught the §11 #52 post-restore capture-window race — reported, not masked) |
...
| `test_oneclick_launch_live.py` | …have the app own the sidecar lifecycle without killing agents? | ✅ FEASIBLE — modeled in Python AND shipped in Electron main (§11 #20; adopt-or-spawn + styled close dialog e2e-proven in the real window, 2026-07-16) |
