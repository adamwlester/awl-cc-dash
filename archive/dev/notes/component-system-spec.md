...
> **⛔ RETIRED (2026-06-29) — historical record only.** This spec has been **executed and fully absorbed** into the five permanent design-source files: values → [`design/tokens.css`](../../design/tokens.css); the shared component CSS → [`design/styles.css`](../../design/styles.css); `data-comp` names + `data-status` markers → [`design/mockup.html`](../../design/mockup.html); the visual catalog + blurbs → [`design/gallery.html`](../../design/gallery.html); and the naming / token / status / behavior rules + the Open Questions register → [`design/DESIGN.md`](../../design/DESIGN.md) (see its **Component system** section). Those files — not this spec — are now the source of truth. Nothing of value lives only here anymore; this document is kept solely as a planning-history record. Do **not** edit it to drive further work.
