...
> Archived, immutable DEVLOG history (oldest -> newest). Rotated out of the root `DEVLOG.md` on
> 2026-06-23 to keep the active log small (see the **Rotation** rule in the DEVLOG header and the
> **Archived history** index there). Entries are **verbatim** -- original `### ...` headings and
> `Files:` lines preserved. Never edit archived entries; record new history in `DEVLOG.md`.
>
> Span: the dashboard design push (ui-concept v5 -> v9p13) and the start of the migration into
> `awl-cc-dash`, from 2026-06-13 07:50 through the 2026-06-21 08:20 entry. 117 entries.
