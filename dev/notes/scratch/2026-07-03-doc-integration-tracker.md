...
- **Q7 — turns-by-tool breakdown → deprioritized / parked** *(tentative)* — no spike now; total turn count suffices meanwhile. Operator drift note: at pickup, re-frame the bucket list against the planned reduction of feed block/filter types (the operator believes some listed blocks are being trimmed or never existed), so the current bucket vocabulary is not settled.
