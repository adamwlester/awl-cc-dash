...
1. **Projects Tab + Close Flow** *(D)*: Add the Projects surface to the design system per the settled model: a **Projects tab first** in the Settings step-in tab row (Projects · Setups · Usage · MCP · Plugins · Config); a known-projects list (registry rows — name, path, agent count, last-opened — with an **Open** action per row) plus **"Open other folder…"** for registering a new project; the open project as an **active-project card pinned on top** (name · repo path · agent count · last-opened) with a **Close project** action, the open row marked in the list, and **one project at a time** — the other rows' Open actions disable while a project is open; the close confirm with **exactly two buttons**, each carrying a one-line explanation (**"Close"** = agents keep running in tmux, reopening reattaches / **"Close & stop agents"** = ends the project's agent sessions first), Cancel/Esc dismissing and either choice returning to the no-project state; a small active-project chip in the topbar/footbar (display + shortcut only — not in the snippet). The empty/no-project-open state ships as a labeled gallery variant, **not** in mockup.html — the mockup stays in the project-open state. **Working concept snippet to match** (built + click-verified 2026-07-02): `.scratch/ui-snippets/projects-tab.html` — a standalone interactive mockup of the whole flow on the real `tokens.css`/`styles.css`, every state reachable by interacting (empty → open → confirm → both close branches → register); concept-only, so rebuild properly across the six design files rather than porting verbatim.
2. **Subagent Demo-Message Roster Fit** *(D)*: The nested subagent messages in the Team Feed → Messages demo data (`behavior.js` MSGS) reference subagents that don't exist on their parent's card. Fix + expand: for **max**, keep **only an A1** subagent message (max's card roster has just A1 — drop the current `max › B1` message, which references a non-existent subagent). Then demo a full multi-subagent stream across **two different agents**, not two runs on one agent — two runs closing that close together in time wouldn't fit the timespan of the Messages example cards. So add **one fen example** that includes **all of fen's B subagents** (B1, B2, B3 nested under fen), and **rework one of the existing sandy examples** to include **all of sandy's A subagents** (A1–A6 nested under sandy).
3. **Turns → Timeline Gap** *(D)*: In the Agent panel → Details view, balance the vertical gap between the Turns accordion body (`#turns-bd-panel`) and the **Timeline** section heading (`.sec-h`) so it equals the gap between the Context accordion (`#ctx-panel`) and the **Turns** section heading (`.ro-head`) above it — the two section gaps should read as equal distance.
4. **Picker Accordion Chevron Control** *(D)*: Give the multi-agent select accordions — the Prompt panel's From filter (`#hist-from`) and To/Target (`#prompt-targets`) pickers (`.src-dd.dd.multi.dd--acc`) — a square, equal-aspect-ratio toggle hit area with the same little vertical divider as the feed/history card accordions' expand control (`.fcard-chevbtn`), replacing the current right-floated inline chevron (`.acc-cv`).
