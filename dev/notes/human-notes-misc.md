...
Backlog of changes for the dashboard mockup — the latest `agent-dashboard/design/ui-concept-v8pN.html`
(the highest version number is the live one). Sorted by implementation effort.

**For agents maintaining this list:**
...
- **Numbering.** Sections are lettered A–E; items are a numbered list within each section, so an item's ID is its section letter + list number (e.g. C7). Cross-reference related items by that ID (e.g. "see C17"), and update those refs if you reorder. Loose notes stay unnumbered.
...
- **Loose notes.** The human drops rough notes under "Loose notes" at the bottom. When asked to incorporate them, fold each into the right section (with an ID + header), then delete it from Loose notes so that bucket stays empty for next time.

## A. Next up — current work queue
...
## Loose notes — rough notes for an agent to incorporate later
*(Empty. Add rough notes here; an agent will fold each into the sections above with an ID + inline header, then clear it from this list.)*
