...
`design/` is one system in five files, each owning one thing: **`tokens.css`** = every value · **`styles.css`** = shared component CSS (linked by both the mockup and the gallery) · **`mockup.html`** = the working app surface + each component's `data-comp` name and `data-status` marker · **`gallery.html`** = the static **states catalog** (every component shown once *per visual state*; behaviour lives in the mockup) · **`DESIGN.md`** = the rules and intent. **Read [`design/DESIGN.md`](design/DESIGN.md) before changing any of them.**
...
- **Propagate every change to all the files it touches:** a **value** → `tokens.css`; **component CSS** → `styles.css`; a **new or changed component** → tag it in `mockup.html` (`data-comp`, plus `data-status` if dormant/undecided) **and** add a specimen *per state* to `gallery.html` **and** register its name in `DESIGN.md`; a **rule or intent** → `DESIGN.md`. A change that lands in only one file when it owes others is unfinished.
- **No design value lives outside `tokens.css`, and nothing in `design/` references `TODO.md`.** Verify the result per **Verifying UI changes** below.
