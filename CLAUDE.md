...
- **No design value lives outside `tokens.css`, and nothing in `design/` references the design backlog** (now [`dev/notes/DESIGN_TODO.md`](dev/notes/DESIGN_TODO.md) — moved out of `design/` to keep the design system to its six files). Verify the result per **Verifying UI changes** below.
